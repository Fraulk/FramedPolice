...
async def checkGVPWinner(msg, author):
    ...
    member = author
    if message_count == 20:
        await msg.channel.send(f"Name: {member.name[:1] if member.name != None else 'None'}\nDisplay name: {member.display_name[:1] if member.display_name != None else 'None'}\nGlobal name: {member.global_name[:1] if member.global_name != None else 'None'}\nNick: {member.nick[:1] if member.nick != None else 'None'}")
    if message_count == 35:
        await msg.channel.send(f"Name: {member.name[:2] if member.name != None else 'None'}\nDisplay name: {member.display_name[:2] if member.display_name != None else 'None'}\nGlobal name: {member.global_name[:1] if member.global_name != None else 'None'}\nNick: {member.nick[:1] if member.nick != None else 'None'}")
    if member != None \
